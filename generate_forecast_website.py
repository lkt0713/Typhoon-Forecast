    ...
    if _genesis_entries:
                ...
                <div class="model-panel" data-track="genesis" data-model="{label}"{'' if i == 0 else ' style="display:none;"'}>
                    <div class="genesis-body">
                        <div class="map-image-wrapper genesis-map-wrapper"
                            ...
                            <img src="{img}"
                                 alt="Western Pacific Tropical Cyclone Genesis Potential ({label})"
                        ...
                        <div class="genesis-legend">
                            <p class="map-note" style="margin-top:0;"
                               data-i18n="note.genesis" data-i18n-source="{_genesis_source(label)}">
                                ...
                                pressure; filled once the member reaches tropical-storm force (≥ 34 kt).
        ...
        genesis_section = f"""
            <div class="panel genesis-panel">
                <div class="panel-header">
                    <span class="panel-icon">🌏</span>
                    <span data-i18n="panel.genesis">Western Pacific Tropical Cyclone Genesis Potential — Ensemble Overview</span>
                </div>
                {genesis_tabs_html}
                {genesis_panels_html}
            </div>"""
...
{cards_section}
    ...
    const I18N = {{
      en: {{
        ...
        'note.genesis':     'Circles = ensemble members at each 6-hr step, colored by minimum sea level pressure; filled once the member reaches tropical-storm force (≥ 34 kt). Gray lines = individual ensemble tracks (0–360 h). Data sourced from {{source}}.',
      ...
      zh: {{
        ...
        'note.genesis':     '圓點＝各系集成員每 6 小時的位置，顏色代表海平面最低氣壓；成員達到熱帶風暴強度（≥ 34 kt）後轉為實心。灰線＝各系集成員的個別路徑（0–360 小時）。資料來源：{{source}}。',
